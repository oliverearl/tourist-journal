...
Conference.dataContext = (function ($) {
    ...
    for (let i = 0; i < 4; i++) {
      tx.executeSql("INSERT INTO \"entries\" (\"id\",\"name\", \"notes\", \"photo\", \"date_of_entry\", \"location\") VALUES (?, ?, ?, ?, ?, ?)",
        [null, 'This is a sample entry.', 'This is a sample description.',
          ...
          Date.now(), `(${Math.random()}),${Math.random()})`], insertSuccess, errorDB);
    }
  ...
  const insertSuccess = function (tx, results) {
    console.log(`Inserted ID: ${results.id}`);
  };
  ...
  const insertDatabase = function(form) {
    ...
    // Quickly check that whatever's been passed from the image is valid base64. Otherwise, disregard it
    db.transaction(function(tx) {
      tx.executeSql('INSERT INTO entries (id, name, notes, photo, date_of_entry, location) VALUES (?, ?, ?, ?, ?, ?)',
        [null, form.name, form.notes, form.photo, currentDatetime, currentLocation], createSuccess, errorDB);
